...
// Cho phép đọc dữ liệu JSON từ Roblox gửi lên
...
// Hệ thống nhận diện 3 Sea của Blox Fruits
const validSeas = {
    "2753915549": "Sea 1",
    "4442274612": "Sea 2",
    "7449423635": "Sea 3"
...
// HÀM XỬ LÝ DỮ LIỆU CHUNG (Đảm bảo hứng trọn data)
function handleFruitUpdate(req, res) {
    // Luôn hứng cả 'fruit' (từ Lua mới) và 'fruitName' (đề phòng bản cũ)
    ...
    // 🔒 LỌC SEA: Kiểm tra xem có đúng là room Blox Fruits không
    if (!validSeas[String(placeId)]) {
        console.log(`⚠️ [Web] Từ chối PlaceId lạ: ${placeId}`);
    ...
    console.log(`✅ [Web] Đã lưu Server! JobId: ${jobid} | ${getSeaName(placeId)} | Trái: ${finalFruitName}`);
    return res.status(200).send("Cập nhật thành công Server!");
}

// CHỐNG LỖI LỆCH URL: Roblox bắn vào đường dẫn nào web cũng nhận!
app.post('/', handleFruitUpdate);
app.post('/update-fruit', handleFruitUpdate);

// API lấy dữ liệu dạng JSON cho script Hop của bạn
app.get('/api', (req, res) => res.json(Array.from(fruitServers.values())));

// Tự động xóa server mất tích sau 15 phút không cập nhật
setInterval(() => {
    const now = Date.now();
...
}, 30000); // Quét mỗi 30 giây để dọn rác nhanh hơn

// Giao diện Web theo dõi (Tự động tải lại sau 8 giây)
app.get('/', (req, res) => {
    const dataArray = Array.from(fruitServers.values());
    const finalData = {
        "Total Execute": totalExecute, 
        "by": "tranduykhanh", 
        "sea_filter": "All Seas (1, 2, 3)",
...
app.listen(PORT, () => console.log(`🚀 Web Fruit đang chạy tại port ${PORT} (Sẵn sàng nhận data 100%)`));
